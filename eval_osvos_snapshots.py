...
from eval_online import eval_osvos_online
...
save_dir = './experiments/eval_snapshots/{}-short'.format(count_)
setproctitle('OSVOS eval {}'.format(count_))

for (vid, inst) in vid_inst:
    weights = f'./experiments/deprecated/{count_}-short/{vid}_{inst}_epoch-{it}.pth'
    print("Evaluating video {}, instance {}, with {} points on first frame.".format(vid, inst, count_))
    eval_osvos_online(vid, inst, count, save_dir, gpu_id, weights, int(it))
